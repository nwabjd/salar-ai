...
export type LocalDayKey = (date: Date) => string;
...
function onLocalDay(
  ...
  return dayKey(new Date(value)) === dayKey(now);
}

export function groupBriefing(
  snapshot: BriefingSnapshot,
  now: Date,
  dayKey: LocalDayKey = localDayKey,
): BriefingGroups {
  const today: TodayEntry[] = [
    ...snapshot.calendar
      .filter((item) => onLocalDay(item.startsAt, now, dayKey))
      .map(
        (item): TodayEntry => ({ id: item.id, kind: "calendar", item }),
      ),
    ...snapshot.reminders
      .filter(
        (item) => !item.done && onLocalDay(item.remindAt, now, dayKey),
      )
      .map(
