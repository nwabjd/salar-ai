...
describe("groupBriefing", () => {
  ...
  it("groups Today by the injected local calendar day across a UTC boundary", () => {
    const localDayAtUtcPlusFive = (date: Date) => {
    ...
    const boundaryCalendar: CalendarItem[] = [
      {
        id: "local-today",
        title: "Just after local midnight",
        startsAt: "2026-07-29T19:05:00.000Z",
        endsAt: null,
        location: "",
        allDay: false,
      },
      {
        ...
        startsAt: "2026-07-29T18:55:00.000Z",
        endsAt: null,
        location: "",
        allDay: false,
      },
    ...
    const grouped = groupBriefing(
      {
        generatedAt: "2026-07-29T19:15:00.000Z",
        tasks: [],
        reminders: [],
      ...
      new Date("2026-07-29T19:15:00.000Z"),
      localDayAtUtcPlusFive,
    );

    expect(grouped.today.map((item) => item.id)).toEqual(["local-today"]);
  });
});
