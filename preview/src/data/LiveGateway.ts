...
export class LiveGateway implements SalarGateway {
  ...
  async performConfirmedAction(
    ...
    const declaredError = [result.error, result.detail].find(
      (value): value is string =>
        typeof value === "string" && Boolean(value.trim()),
    );
    const ok = result.ok !== false && !declaredError;
    return {
      ok,
      message:
        declaredError ??
        (typeof result.message === "string"
          ...
          : ok
            ...
            : `${action.label} was not completed.`),
  ...
  private parseSseFrame(frame: string): ChatStreamEvent | null {
    ...
    try {
      ...
      if (type === "approval_request") {
        ...
        if (
          !["approve_command", "acknowledge_alert"].includes(kind) ||
          required.some(
        ...
        return {
          ...
          request: {
            ...
            kind: kind as ApprovalStreamRequest["kind"],
