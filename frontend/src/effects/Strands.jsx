...
export default function Strands({
  colors = ['#FF4242', '#7C3AED', '#06B6D4', '#EAB308'], count = 3, speed = 0.5,
  amplitude = 1, waviness = 1, thickness = 0.7, glow = 2.6, taper = 3, spread = 1,
  hueShift = 0, intensity = 0.6, saturation = 1.5, opacity = 1, scale = 1.5,
  glass = false, refraction = 1, dispersion = 1, glassSize = 1, className = '', style = undefined
}) {
  const propsRef = useRef({});
  propsRef.current = { colors, count, speed, amplitude, waviness, thickness, glow, taper, spread,
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uStrandCount: { value: Math.min(propsRef.current.count, MAX_STRANDS) }, uSpeed: { value: speed },
        uAmplitude: { value: amplitude }, uWaviness: { value: waviness }, uThickness: { value: thickness },
        uGlow: { value: glow }, uTaper: { value: taper }, uSpread: { value: spread },
        uHueShift: { value: hueShift }, uIntensity: { value: intensity }, uOpacity: { value: opacity },
    ...
    const glassProgram = new Program(gl, {
      ...
      uniforms: { uScene: { value: renderTarget.texture }, uResolution: { value: [ctn.offsetWidth, ctn.offsetHeight] },
        uRadius: { value: 0.46 * glassSize }, uRefraction: { value: refraction }, uDispersion: { value: dispersion } }
    });
    const glassMesh = new Mesh(gl, { geometry, program: glassProgram });
    ctn.appendChild(gl.canvas);

    function resize() {
      const width = ctn.offsetWidth, height = ctn.offsetHeight;
    ...
    const update = t => {
      ...
      for (const name of ['Speed','Amplitude','Waviness','Thickness','Glow','Taper','Spread','HueShift','Intensity','Opacity','Scale','Saturation']) {
        program.uniforms[`u${name}`].value = current[name.charAt(0).toLowerCase() + name.slice(1)];
      }
      if (current.glass) {
